...
if __name__ == "__main__":
    ...
    run_git_command("git config user.email \"hwanginho@users.noreply.github.com\" ", repo_dir)

    all_generated_filenames = []

    for target_keyword in config.KEYWORDS:
        ...
        all_generated_filenames.append(os.path.join('output', filename))
    ...
    # Add all generated content and script files to staging
    for f in all_generated_filenames:
        run_git_command(f"git add {f}", repo_dir)

    run_git_command(f"git add {os.path.join('config.py')}", repo_dir) 
    run_git_command(f"git add {os.path.basename(__file__)}", repo_dir) 
    run_git_command(f"git add .gitignore", repo_dir) 

    # Commit the changes
    commit_message = f"feat: Generate dummy content for all keywords and push to GitHub Pages"
    run_git_command(f"git commit -m \"{commit_message}\" ", repo_dir)

    # Fetch remote changes and rebase local changes on top
